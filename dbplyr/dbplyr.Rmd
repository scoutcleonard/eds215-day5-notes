...
#you aren't getting back a handle, but a handle to the table..? 
```
...
#to figure it out, we can't use nrow, because we are talking to a database. we need to use sql. example:
nrow(flights_db)
...
#wherever dbply takes over a routine that dplyr can do, it looks like dbplyr
```
